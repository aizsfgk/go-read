...
// Integrated network poller (platform-independent part).      /// 完整的网络循环器，平台独立部分
// A particular implementation (epoll/kqueue/port/AIX/Windows) /// 一个特殊的实现 epoll/kqueue等
// must define the following functions:                        /// 必须定义下边的实现
//
// func netpollinit()
...
var (
	netpollInitLock mutex   /// netpoll初始化互斥锁
	netpollInited   uint32  /// netpoll 是否已经初始化
...
func poll_runtime_pollOpen(fd uintptr) (*pollDesc, int) {
	pd := pollcache.alloc() /// 每打开一个文件，就从缓存链表上拿一个pollDesc
	lock(&pd.lock) // 锁住
...
func (c *pollCache) alloc() *pollDesc {
	...
	pd := c.first
	c.first = pd.link
	lockInit(&pd.lock, lockRankPollDesc) /// 初始化pd lock
